...
import quaero.components.select.*;
...
public class QueryBuilder {
	...
	public Query build() {
		...
		if (!rootFilters.isEmpty()) {
			query.setFilter(
					rootFilters.size() == 1 ? rootFilters.get(0) : buildFilterArray(rootOperation, rootFilters));
		}
	...
	private static IFilter buildFilterArray(final FilterArrayObject.FilterOperationType operation,
